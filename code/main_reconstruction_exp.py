...
def main():
    ...
    if args.with_tracking:
        wandb.init(project='Worst case experiment', name=args.run_name)
        wandb.config.update(args)
        table = wandb.Table(columns=['original_image', 'recon_image', 'diff_image', 'original_latents_visualized', 'recon_latents_visualized', 'diff_latents_visualized', 'error_TOT', 'error_OTO', 'error_middle', 'output_loss', 'inf_time', 'inv_time', 'seed', 'prompt'])
    ...
    if args.with_tracking:
        ...
        wandb.log({'mean_T0T' : mean_TOT,'std_T0T' : std_TOT,'mean_0T0' : mean_OTO,'std_0T0' : std_OTO, 'mean_middle' : mean_middle_error, 'std_middle' : std_middle_error, 'mean_loss' : mean_loss, 'std_loss' : std_loss, 'guidance_scale': args.guidance_scale, 'inv_time': inv_time_avg, 'prompt': prompt})
        wandb.finish()
    else:
